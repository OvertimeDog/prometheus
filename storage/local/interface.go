...
type Querier interface {
	...
	QueryInstant(ts model.Time, stalenessDelta time.Duration, matcherSets ...*metric.LabelMatcher) ([]SeriesIterator, error)
	...
	// least one label matcher that does not match the empty string. Within one
	// set of matchers, the intersection of matching series is computed. The
	// final return value will be the union of the per-set results.
	// The times from and through are hints for the storage to optimize the
	// search. The storage MAY exclude metrics that have no samples in the
	// specified interval from the returned map. In doubt, specify model.Earliest
	// for from and model.Latest for through.
	MetricsForLabelMatchers(from, through model.Time, matchers ...metric.LabelMatchers) ([]metric.Metric, error)
	...
	LastSampleForLabelMatchers(cutoff model.Time, matchers ...metric.LabelMatchers) (model.Vector, error)
